...
```powershell
$releaseName = "reimer-v0.1.0-x86_64-pc-windows-msvc"
...
tar -xzf reimer-v0.1.0-x86_64-unknown-linux-gnu.tar.gz \
  --strip-components=1 \
...
```powershell
(Get-FileHash .\reimer-v0.1.0-x86_64-pc-windows-msvc.zip -Algorithm SHA256).Hash
```

```bash
sha256sum -c reimer-v0.1.0-x86_64-unknown-linux-gnu.tar.gz.sha256
```
