...
| Standard library plan | `alloc`, `c`, `collections`, `fs`, `io`, `math`, `string`, `target`, `thread`, `job`, and `tensor`; allocator-aware concatenation, primitive formatting, Unicode queries and full scalar case mappings | Interpolated strings with `Display`/`Debug` |
