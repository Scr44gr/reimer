...
| M5 - FFI | Complete | `@link(...) extern "C"` blocks, preserved symbols, `@repr(C)`, `cstr` and validated `c"..."` literals, ABI-safe types, mandatory `unsafe` calls, safe wrappers, JIT dynamic loading, COFF `/DEFAULTLIB` directives, and LLD-linked executables; transparent target-correct aliases for C scalars, typed null pointers, ABI-safe C callbacks, checked `unsafe` casts from loaded addresses to typed functions, and `@repr(C)` pointer/count buffers in `std::c`; by-value aggregate classifiers cover Microsoft x64, System V AMD64, AAPCS64, and Apple AArch64; generated wgpu-native bindings exercise large descriptor and callback-info layouts, while real Windows tests cover C aggregate execution, headless WebGPU initialization, mapped buffers, and an SDL3 presentation surface | C unions, C variadics/`va_list`, 32-bit aggregate ABIs, and big-endian aggregate ABIs remain |
| M6 - Generics | Complete | Type, variadic type-pack, and `const` parameters; defaults, per-pack bounds, mapped tuple/array expansion, `where` clauses, and compile-time type-addressable tuple splitting; demand-driven cached monomorphization of functions, structs, enums, associated functions, and methods; inference from arguments, tuple packs, and expected returns; traits, supertraits, basic coherence, contract validation, `Copy` markers, and static dispatch; executable `examples/m6_methods.reim`, `examples/m6_generics.reim`, and `examples/variadic_registry.reim`, verified by native execution | - |
...
| M10 - Comptime | Complete | Typed global constants and const generics; deterministic `comptime` functions/blocks with limits; `@repr`, `@derive`, `@align`, `@inline`, `@test`, and `@must_use`; structural derives `Copy`, `Clone`, `Debug`, `Eq`, `Hash`, `Default`, and padding-checked `Pod`; library-defined marker derives with supertrait validation; typed `size_of`, `align_of`, and `meta::*` reflection; layout shared by reflection and Cranelift; isolated unit tests and JIT-verified `examples/m10_comptime.reim` | `@inline` is a frontend hint; advanced inlining policy and runtime reflection are outside v0.1 |
...
| First integrated demo | `examples/m5_sdl_opengl.reim` remains the focused SDL3/OpenGL compiler fixture; `examples/imgui_demo` exercises the manifest-backed SDL3/OpenGL runtime path with transitive native dependency resolution and staged runtime files | - |
