...
| M5 - FFI | Complete | `@link(...) extern "C"` blocks, preserved symbols, `@repr(C)`, `cstr` and validated `c"..."` literals, ABI-safe types, mandatory `unsafe` calls, safe wrappers, JIT dynamic loading, COFF `/DEFAULTLIB` directives, and LLD-linked executables; transparent target-correct aliases for C scalars, typed null pointers, ABI-safe C callbacks, checked `unsafe` casts from loaded addresses to typed functions, `@repr(C)` pointer/count buffers in `std::c`, and tested Windows x64 by-value C aggregate parameters and returns; `examples/m5_ffi.reim` and `examples/m5_c_types.reim` test scalar calls and ABI aliases, while `examples/m5_sdl_window.reim` opens a real SDL3 window and `examples/m5_sdl_opengl.reim` integrates SDL3, OpenGL, tensor storage, an explicit allocator, `Result`, and `defer`; reproducible official downloads with SHA-256 verification in the SDL scripts | C unions, C variadics/`va_list`, and by-value aggregate classification on non-Windows targets remain |
